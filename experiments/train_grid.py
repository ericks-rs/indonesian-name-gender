...
OUT_DIR = RESULTS_DIR / "tables" / "multiseed"
...
def run(name, seed):
    ...
    ext = evaluate(model, ext_dl, crit, key)
...
rows = []
runs_csv = OUT_DIR / "multiseed_runs.csv"
total = len(MODELS) * len(SEEDS)
i = 0
for name in MODELS:
    for seed in SEEDS:
        i += 1
        m, x, ep, dur, npar = run(name, seed)
        rows.append({"Model": name, "Seed": seed, "Params": npar,
                     "Accuracy": round(m["accuracy"], 6), "Precision": round(m["precision"], 6),
...
agg = []
for name, g in runs.groupby("Model", sort=False):
    rec = {"Model": name, "Params": int(g.Params.iloc[0]), "n_seeds": n}
    for metric in ["Accuracy", "Precision", "Recall", "F1"]:
        v = g[metric].values
        mean, sd = v.mean(), v.std(ddof=1)
        half = tcrit * sd / np.sqrt(n)
        rec[f"{metric}_mean"] = round(mean, 4)
        rec[f"{metric}_std"] = round(sd, 4)
        rec[f"{metric}_ci95_lo"] = round(mean - half, 4)
        rec[f"{metric}_ci95_hi"] = round(mean + half, 4)
    rec["F1_min"] = round(g.F1.min(), 4)
    rec["F1_max"] = round(g.F1.max(), 4)
    agg.append(rec)
...
for a, b in pairs:
    ...
    cmp_rows.append({
        "Char_model": a, "Word_model": b,
        "Char_F1_mean": round(va.mean(), 4), "Char_F1_std": round(va.std(ddof=1), 4),
        "Word_F1_mean": round(vb.mean(), 4), "Word_F1_std": round(vb.std(ddof=1), 4),
        "Diff_pp": round(diff * 100, 3),
        "Welch_t": round(float(t_ind.statistic), 3), "Welch_p": float(t_ind.pvalue),
        "Paired_t": round(float(t_rel.statistic), 3), "Paired_p": float(t_rel.pvalue),
        "Cohens_d": round(float(diff / pooled), 2) if pooled > 0 else np.nan,
    })
cmp = pd.DataFrame(cmp_rows)
cmp.to_csv(OUT_DIR / "multiseed_char_vs_word.csv", index=False)

pd.set_option("display.width", 240)
print("\n" + "=" * 120)
print(f"MULTI-SEED SUMMARY ({n} seeds: {SEEDS})")
print("=" * 120)
show = ["Model", "Params", "F1_mean", "F1_std", "F1_ci95_lo", "F1_ci95_hi", "F1_min", "F1_max",
        "Accuracy_mean", "Accuracy_std"]
print(summary[show].to_string(index=False))

print("\n" + "=" * 120)
print("CHARACTER vs WORD ACROSS SEEDS")
print("=" * 120)
d = cmp.copy()
for c in ["Welch_p", "Paired_p"]:
    d[c] = d[c].apply(lambda v: f"{v:.3e}" if v < 1e-3 else f"{v:.4f}")
